...
Первый шаг сброса счётчика неверных вводов PIN пластиковой карты: сервер отправляет одноразовый код на email учётной записи. Второй шаг — `reset_pin` с этим кодом.
...
```text
Код отправлен на email учётной записи. Передайте его в reset_pin(code=...)
```
...
**Почему:** Запрос отправлен повторно, пока предыдущий такой же запрос ещё обрабатывается.

**Что делать:** Дождитесь письма с кодом и используйте его в `reset_pin`.
...
- Раздел спецификации 1.1.60: «Запрос одноразового кода для сброса попыток ввода PIN карты». Запрос в спецификации: `POST http://localhost/vip/v2/cards/{card_id}/verifyPIN`.
...
```text
POST: http://localhost/vip/v2/cards/4534535/verifyPIN
```

## Что важно знать

- Сброс нужен, когда пластиковую карту заблокировали из-за неверных вводов PIN на АЗС. Код приходит на email, привязанный к учётной записи пользователя API, а не водителю.
- Метод не тарифицируется, но не вызывайте его в цикле: дождитесь письма и переходите к `reset_pin`.
