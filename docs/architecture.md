---
description: Архитектура apisdkopti24, слои выполнения запросов, сервисы, transport и контрактные metadata.
---
...
`client.registry` используется как единый каталог для инспекции маршрутов и
metadata. Он создаётся самим SDK и не принимается через конструктор
`APIClient`, поэтому больше не создаёт ложного ожидания, что пользовательский
registry способен перенаправить фактический endpoint метода.

Доменные сервисы выполняют собственные типизированные константы
`OperationSpec`. Для тестирования HTTP-взаимодействия внедряйте `transport`.


## Граница диагностики ошибок

`errors.py` нормализует непроверенный ответ в предметное исключение, сохраняя
raw payload отдельно от публичного контекста. `error_reporting.py` переводит
исключения в безопасный `ErrorDescriptor` и audit-события, а `logger.py`
форматирует текст и JSON. Общая дополнительная очистка находится в независимом
`sanitization.py`; прежние импорты из `utils.py` сохранены.
Неизвестная серверная metadata не становится текстом журнала. Сбой обработчика
журнала не меняет результат операции; недоставка учитывается отдельно.
