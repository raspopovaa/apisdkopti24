---
description: Асинхронный Python SDK apisdkopti24 для Opti24 API — API топливных карт АЗС (ОПТИ 24) и QR-платежей.
---

# apisdkopti24

Асинхронная Python-библиотека для **Opti24 API** — корпоративного **API топливных
карт АЗС** (сервис ОПТИ 24). SDK покрывает авторизацию, управление картами,
договорами, транзакциями, отчётами, пользователями, виртуальными картами и оплату
по QR.
...
!!! info "Текущая версия"
    Документация соответствует `apisdkopti24 3.3.11` и Python
    `>=3.11,<3.15`.

!!! warning "Статус проекта"
    Проект находится в разработке и ещё не использовался в production.
    Проверяйте интеграцию на тестовом окружении перед применением SDK.
...
- автоматическая сверка 89 операций с независимым YAML-контрактом.
...
- [Пакет apisdkopti24 на PyPI](https://pypi.org/project/apisdkopti24/)
