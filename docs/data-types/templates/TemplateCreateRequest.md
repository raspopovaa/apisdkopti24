---
description: "Поля и правила Pydantic-валидации модели TemplateCreateRequest."
---
...
!!! info "Назначение Pydantic"
    Тип модели: **request**. Правила ниже применяются, когда вызывающий код явно создаёт `TemplateCreateRequest` или вызывает `TemplateCreateRequest.model_validate(payload)`. Наличие request-модели не означает, что каждый метод SDK автоматически создаёт её: фактический входной контракт определяется сигнатурой соответствующего сервисного метода.
...
| `type` | `Literal[Limit, Wallet]` | `string` | Да | Нет | `—` | `—` | допустимые значения: 'Limit', 'Wallet' | Допускаются только значения: 'Limit', 'Wallet' | Тип создаваемого шаблона |
