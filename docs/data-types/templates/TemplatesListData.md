---
description: "Поля и правила Pydantic-валидации модели TemplatesListData."
---
# `TemplatesListData`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `total_count` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Общее количество найденных шаблонов |
| `result` | `list[TemplateItem] \| None` | `array[object (TemplateItem)] \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: list[TemplateItem], None | Список найденных шаблонов ВК |
