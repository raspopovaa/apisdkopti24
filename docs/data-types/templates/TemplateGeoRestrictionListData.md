...
| `result` | `list[TemplateGeoRestriction]` | `array[object (TemplateGeoRestriction)]` | Да | Нет | `—` | `—` | — | Проверяется как список; каждый элемент проверяется как TemplateGeoRestriction. | Список геоограничителей шаблона |
...
- [`TemplateGeoRestriction`](TemplateGeoRestriction.md)
