...
| `result` | `list[TemplateLimit]` | `array[object (TemplateLimit)]` | Да | Нет | `—` | `—` | — | Проверяется как список; каждый элемент проверяется как TemplateLimit. | Список лимитов шаблона |
