---
description: "Поля и правила Pydantic-валидации модели LimitTime."
---
# `LimitTime`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `type` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Тип периода лимита (например, 3 — день, 5 — месяц) |
| `number` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Количество единиц выбранного периода |
