---
description: "Поля и правила Pydantic-валидации модели LimitTerm."
---
# `LimitTerm`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `days` | `str \| None` | `string \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: str, None | Маска дней действия лимита (например, '1111100') |
| `type` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Тип временного ограничения |
| `time` | `LimitTermTime \| None` | `object (LimitTermTime) \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: LimitTermTime, None | Временные границы лимита |
...
- [`LimitTermTime`](LimitTermTime.md)
