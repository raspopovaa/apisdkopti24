---
description: "Поля и правила Pydantic-валидации модели LimitAmount."
---
# `LimitAmount`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `unit` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Единица измерения (например, 'LIT') |
| `value` | `float` | `number` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как float. | Количество или объем в единицах измерения |
