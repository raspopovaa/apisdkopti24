...
| `result` | `list[TransactionItemV2]` | `array[object (TransactionItemV2)]` | Да | Нет | `—` | `—` | — | Проверяется как список; каждый элемент проверяется как TransactionItemV2. | Список транзакций (v2) |
...
- [`TransactionItemV2`](TransactionItemV2.md)
