---
description: "Поля и правила Pydantic-валидации модели MPCListData."
---
# `MPCListData`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `total_count` | `int` | `integer` | Да | Нет | `—` | `—` | минимум: 0 | Значение преобразуется и проверяется как int. | Количество найденных МПК |
| `result` | `list[MPCItem]` | `array[object (MPCItem)]` | Да | Нет | `—` | `—` | — | Проверяется как список; каждый элемент проверяется как MPCItem. | Список выпущенных МПК |
