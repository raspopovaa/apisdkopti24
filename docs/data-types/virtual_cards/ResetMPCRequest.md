---
description: "Поля и правила Pydantic-валидации модели ResetMPCRequest."
---
# `ResetMPCRequest`
...
!!! info "Назначение Pydantic"
    Тип модели: **request**. Правила ниже применяются, когда вызывающий код явно создаёт `ResetMPCRequest` или вызывает `ResetMPCRequest.model_validate(payload)`. Наличие request-модели не означает, что каждый метод SDK автоматически создаёт её: фактический входной контракт определяется сигнатурой соответствующего сервисного метода.
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `type` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Тип операции сброса ('ResetCounterCode' и т.п.) |
