---
description: "Поля и правила Pydantic-валидации модели RerunVirtualCardReleaseResponse."
---
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `status` | `StatusModel` | `object (StatusModel)` | Да | Нет | `—` | `—` | — | Вложенный объект рекурсивно проверяется моделью StatusModel. | Статус перезапуска выпуска карты |
| `data` | `VirtualCardData` | `object (VirtualCardData)` | Да | Нет | `—` | `—` | — | Вложенный объект рекурсивно проверяется моделью VirtualCardData. | Обновлённая информация о виртуальной карте |
| `timestamp` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Время выполнения запроса (Unix Timestamp) |
