---
description: "Поля и правила Pydantic-валидации модели SimpleActionResponse."
---
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `status` | `StatusModel` | `object (StatusModel)` | Да | Нет | `—` | `—` | — | Вложенный объект рекурсивно проверяется моделью StatusModel. | Статус выполнения операции |
| `data` | `bool` | `boolean` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как bool. | Результат операции (True — успешно) |
