...
| `number` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Номер виртуальной карты |
| `carrier` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Тип носителя, обычно 'Virtual Card' |
| `product` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Тип продукта карты ('wallet' или 'limit') |
| `status` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Статус карты (например, 'Active', 'Blocked', 'Pending') |
