---
description: "Поля и правила Pydantic-валидации модели MPCConfirmRequest."
---
# `MPCConfirmRequest`
...
!!! info "Назначение Pydantic"
    Тип модели: **request**. Правила ниже применяются, когда вызывающий код явно создаёт `MPCConfirmRequest` или вызывает `MPCConfirmRequest.model_validate(payload)`. Наличие request-модели не означает, что каждый метод SDK автоматически создаёт её: фактический входной контракт определяется сигнатурой соответствующего сервисного метода.
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `code` | `str` | `string` | Да | Нет | `—` | `—` | минимальная длина: 1 | Значение преобразуется и проверяется как str. | — |
