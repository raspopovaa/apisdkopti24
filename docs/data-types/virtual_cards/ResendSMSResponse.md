---
description: "Поля и правила Pydantic-валидации модели ResendSMSResponse."
---
# `ResendSMSResponse`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `status` | `StatusModel` | `object (StatusModel)` | Да | Нет | `—` | `—` | — | Вложенный объект рекурсивно проверяется моделью StatusModel. | Статус запроса на повторную отправку СМС-кода |
| `data` | `bool` | `boolean` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как bool. | Результат операции (True — СМС отправлено успешно) |
