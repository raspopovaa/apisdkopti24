---
description: "Поля и правила Pydantic-валидации модели DeleteVirtualCardResponse."
---
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `status` | `StatusModel` | `object (StatusModel)` | Да | Нет | `—` | `—` | — | Вложенный объект рекурсивно проверяется моделью StatusModel. | Статус удаления виртуальной карты |
| `data` | `bool` | `boolean` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как bool. | Результат удаления карты (True — успешно) |
