---
description: "Поля и правила Pydantic-валидации модели ConfirmVirtualCardResponse."
---
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `status` | `StatusModel` | `object (StatusModel)` | Да | Нет | `—` | `—` | — | Вложенный объект рекурсивно проверяется моделью StatusModel. | Статус подтверждения выпуска |
| `data` | `bool` | `boolean` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как bool. | Результат подтверждения (True — успешно) |
