---
description: "Поля и правила Pydantic-валидации модели DeleteMPCResponse."
---
# `DeleteMPCResponse`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `status` | `StatusModel` | `object (StatusModel)` | Да | Нет | `—` | `—` | — | Вложенный объект рекурсивно проверяется моделью StatusModel. | Статус удаления мобильного профиля карты (МПК) |
| `data` | `bool` | `boolean` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как bool. | Результат удаления (True — успешно) |
