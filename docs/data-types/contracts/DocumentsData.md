...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `total_count` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Количество найденных документов |
| `result` | `list[DocumentItem] \| None` | `array[object (DocumentItem)] \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: list[DocumentItem], None | Список найденных документов |
