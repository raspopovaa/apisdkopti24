---
description: "Секция 'data' в ответе списка счетов."
---
# `InvoicesData`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `total_count` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Количество найденных счетов |
| `result` | `list[InvoiceItem] \| None` | `array[object (InvoiceItem)] \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: list[InvoiceItem], None | Список счетов на оплату |
