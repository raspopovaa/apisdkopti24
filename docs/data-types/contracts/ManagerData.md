---
description: "Данные менеджера по сопровождению договора"
---
# `ManagerData`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `email` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Email менеджера |
| `first_name` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Имя менеджера |
| `last_name` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Фамилия менеджера |
| `middle_name` | `str \| None` | `string \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: str, None | Отчество менеджера |
| `work_phone` | `str \| None` | `string \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: str, None | Рабочий телефон менеджера |
