---
description: "Результат действий с приглашениями (создание, продление, повторная отправка)"
---
# `InviteActionResult`
...
| `attempts` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Количество попыток отправки |
| `expired_at` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Дата истечения срока действия ссылки (timestamp) |
