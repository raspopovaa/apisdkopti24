---
description: "Строгий элемент запроса установки товарного ограничителя."
---
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `id` | `str \| None` | `string \| null` | Нет | Да | `None` | `—` | минимальная длина: 1; — | Значение должно соответствовать одному из типов: str, None | ID изменяемого ограничителя |
...
| `product_type` | `str` | `string` | Да | Нет | `—` | `productType` | минимальная длина: 1 | Значение преобразуется и проверяется как str. | ID типа продукта |
...
| `restriction_type` | `Literal[1, 2]` | `integer` | Да | Нет | `—` | `—` | допустимые значения: 1, 2 | Допускаются только значения: 1, 2 | Тип ограничителя: 1 — разрешающий, 2 — запрещающий |
