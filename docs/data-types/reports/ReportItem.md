---
description: "Описание доступного отчета (v2)."
---
# `ReportItem`
...
| `name` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Название отчета |
| `formats` | `list[str]` | `array[string]` | Да | Нет | `—` | `—` | — | Проверяется как список; каждый элемент проверяется как str. | Список поддерживаемых форматов (pdf, xlsx, csv и т.д.) |
| `parameters` | `list[ReportParameter]` | `array[object (ReportParameter)]` | Да | Нет | `—` | `—` | — | Проверяется как список; каждый элемент проверяется как ReportParameter. | Список параметров отчета |
...
- [`ReportParameter`](ReportParameter.md)
