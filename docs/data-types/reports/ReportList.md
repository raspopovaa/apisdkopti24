---
description: "Ответ метода /v2/reports — список доступных отчетов."
---
# `ReportList`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `total_count` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Количество доступных отчетов |
| `result` | `list[ReportItem] \| None` | `array[object (ReportItem)] \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: list[ReportItem], None | Массив отчетов |
