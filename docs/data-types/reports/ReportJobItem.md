---
description: "Элемент списка заказанных отчетов."
---
# `ReportJobItem`
...
| `available_after` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Количество секунд до доступности отчета |
