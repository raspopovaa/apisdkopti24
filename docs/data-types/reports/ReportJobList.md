---
description: "Ответ со списком заказанных отчетов (v1/v2)."
---
# `ReportJobList`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `total_count` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Количество найденных отчетов |
| `result` | `list[ReportJobItem] \| None` | `array[object (ReportJobItem)] \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: list[ReportJobItem], None | Список заказанных отчетов |
...
- [`ReportJobItem`](ReportJobItem.md)
