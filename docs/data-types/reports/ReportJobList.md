...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `total_count` | `int \| None` | `integer \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: int, None | Количество найденных отчетов |
| `result` | `list[ReportJobItem]` | `array[object (ReportJobItem)]` | Да | Нет | `—` | `—` | — | Проверяется как список; каждый элемент проверяется как ReportJobItem. | Список заказанных отчетов |
