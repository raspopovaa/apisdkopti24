---
description: "Основной объект данных для списка карт (v2)."
---
# `CardsV2Data`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `total_count` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Общее количество найденных карт |
| `result` | `list[CardV2Item] \| None` | `array[object (CardV2Item)] \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: list[CardV2Item], None | Список карт договора |
