---
description: "Поля и правила Pydantic-валидации модели CardDetailData."
---
# `CardDetailData`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `total_count` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Количество записей |
| `result` | `list[CardDetail] \| None` | `array[object (CardDetail)] \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: list[CardDetail], None | Список карт |
