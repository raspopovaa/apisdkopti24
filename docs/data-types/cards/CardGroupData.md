---
description: "Поля и правила Pydantic-валидации модели CardGroupData."
---
# `CardGroupData`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `total_count` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Количество карт в группе |
| `result` | `list[CardGroupInfo] \| None` | `array[object (CardGroupInfo)] \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: list[CardGroupInfo], None | Список карт в группе |
...
- [`CardGroupInfo`](CardGroupInfo.md)
