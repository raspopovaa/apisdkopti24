---
description: "Поля и правила Pydantic-валидации модели ResetPinRequest."
---
# `ResetPinRequest`
...
!!! info "Назначение Pydantic"
    Тип модели: **request**. Правила ниже применяются, когда вызывающий код явно создаёт `ResetPinRequest` или вызывает `ResetPinRequest.model_validate(payload)`. Наличие request-модели не означает, что каждый метод SDK автоматически создаёт её: фактический входной контракт определяется сигнатурой соответствующего сервисного метода.
