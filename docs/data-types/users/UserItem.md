...
| `middle_name` | `str \| None` | `string \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: str, None | Отчество пользователя |
| `date` | `str \| None` | `string \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: str, None | Дата рождения |
| `position` | `str \| None` | `string \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: str, None | Должность или UUID должности |
| `role` | `UserRole` | `object (UserRole)` | Да | Нет | `—` | `—` | — | Вложенный объект рекурсивно проверяется моделью UserRole. | Роль пользователя |
| `active` | `bool` | `boolean` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как bool. | Активен ли пользователь |
