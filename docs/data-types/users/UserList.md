---
description: "Поля и правила Pydantic-валидации модели UserList."
---
# `UserList`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `total_count` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Общее количество пользователей |
| `result` | `list[UserItem] \| None` | `array[object (UserItem)] \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: list[UserItem], None | Список пользователей |
