---
description: "Строгий одиночный идентификатор path-параметра."
---
# `ResourcePath`
...
!!! info "Назначение Pydantic"
    Тип модели: **request**. Правила ниже применяются, когда вызывающий код явно создаёт `ResourcePath` или вызывает `ResourcePath.model_validate(payload)`. Наличие request-модели не означает, что каждый метод SDK автоматически создаёт её: фактический входной контракт определяется сигнатурой соответствующего сервисного метода.
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `resource_id` | `str` | `string` | Да | Нет | `—` | `—` | минимальная длина: 1 | Значение преобразуется и проверяется как str. | — |
