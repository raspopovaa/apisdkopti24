...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `code` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Код выполнения API-операции |
| `message` | `str \| None` | `string \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: str, None | Текст статуса API-операции |
| `errors` | `list[dict[str, object]] \| None` | `array[object] \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: list[dict[str, object]], None | Массив ошибок; отсутствует, если операция завершилась без ошибок |
