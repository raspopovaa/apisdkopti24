...
| `value` | `float` | `number` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как float. | Сумма лимита |
| `used` | `float` | `number` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как float. | Использованный объём лимита |
