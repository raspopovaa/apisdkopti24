---
description: "Поля и правила Pydantic-валидации модели LimitSumRequest."
---
# `LimitSumRequest`
...
!!! info "Назначение Pydantic"
    Тип модели: **request**. Правила ниже применяются, когда вызывающий код явно создаёт `LimitSumRequest` или вызывает `LimitSumRequest.model_validate(payload)`. Наличие request-модели не означает, что каждый метод SDK автоматически создаёт её: фактический входной контракт определяется сигнатурой соответствующего сервисного метода.
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `currency` | `str` | `string` | Да | Нет | `—` | `—` | минимальная длина: 1 | Значение преобразуется и проверяется как str. | Код валюты |
| `value` | `float` | `number` | Да | Нет | `—` | `—` | строго больше: 0 | Значение преобразуется и проверяется как float. | Размер денежного лимита |
