---
description: "Временной диапазон действия лимита."
---
# `LimitTermTime`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `from_` | `str` | `string` | Да | Нет | `—` | `from` | — | Значение преобразуется и проверяется как str. | Время начала действия лимита (HH:MM) |
| `to` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Время окончания действия лимита (HH:MM) |
