...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `number` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Период в числовом виде (например, 3) |
| `type` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Тип периода (например, 7 — неделя) |
