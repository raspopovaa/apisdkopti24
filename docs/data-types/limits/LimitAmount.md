---
description: "Объёмный лимит (например, литры)."
---
# `LimitAmount`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `value` | `float` | `number` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как float. | Установленное значение лимита |
| `used` | `float` | `number` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как float. | Использованное значение лимита |
| `unit` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Единица измерения (например, 'LIT' или 'RUB') |
