---
description: "Поля и правила Pydantic-валидации модели LimitTimeRequest."
---
# `LimitTimeRequest`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `number` | `int` | `integer` | Да | Нет | `—` | `—` | строго больше: 0 | Значение преобразуется и проверяется как int. | Количество периодов |
| `type` | `Literal[2, 3, 4, 5, 6, 7]` | `integer` | Да | Нет | `—` | `—` | допустимые значения: 2, 3, 4, 5, 6, 7 | Допускаются только значения: 2, 3, 4, 5, 6, 7 | Тип периода |
