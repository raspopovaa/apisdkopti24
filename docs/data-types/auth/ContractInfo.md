...
| `cards_count` | `int` | `integer` | Нет | Нет | `0` | `—` | — | Значение преобразуется и проверяется как int. | Количество карт на договоре |
| `one_price` | `bool` | `boolean` | Нет | Нет | `False` | `—` | — | Значение преобразуется и проверяется как bool. | Признак единой цены |
