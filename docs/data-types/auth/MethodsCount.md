---
description: "Поля и правила Pydantic-валидации модели MethodsCount."
---
# `MethodsCount`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `all` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Общее количество методов |
| `cards` | `int \| None` | `integer \| null` | Нет | Да | `0` | `—` | — | Значение должно соответствовать одному из типов: int, None | Методы, связанные с картами |
| `cardgroups` | `int \| None` | `integer \| null` | Нет | Да | `0` | `—` | — | Значение должно соответствовать одному из типов: int, None | Методы, связанные с группами карт |
| `card` | `int \| None` | `integer \| null` | Нет | Да | `0` | `—` | — | Значение должно соответствовать одному из типов: int, None | Методы, связанные с одной картой |
