---
description: "Основные данные о финальных ценах"
---
# `FinalPricesData`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `total_count` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Количество товарных позиций в ответе |
| `goods` | `list[FinalPriceItem]` | `array[object (FinalPriceItem)]` | Да | Нет | `—` | `—` | — | Проверяется как список; каждый элемент проверяется как FinalPriceItem. | Список товарных позиций с рассчитанными финальными ценами |
...
- [`FinalPriceItem`](FinalPriceItem.md)
