---
description: "Описание товарной позиции для проверки возможности покупки"
---
# `PurchaseGoodItem`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `code` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Код товара (SKU или PLU на АЗС) |
| `quantity` | `float` | `number` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как float. | Количество товара для покупки |
| `price` | `float` | `number` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как float. | Цена за единицу товара |
