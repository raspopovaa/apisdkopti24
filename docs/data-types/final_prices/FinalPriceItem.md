---
description: "Информация о финальной цене товара на АЗС"
---
# `FinalPriceItem`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `code` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Код товарной позиции |
| `price` | `float` | `number` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как float. | Финальная цена товара (с учетом всех скидок и тарифов) |
