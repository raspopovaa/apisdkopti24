...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `total_count` | `int \| None` | `integer \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: int, None | Количество элементов в справочнике |
| `result` | `list[DictionaryItem] \| None` | `array[object (DictionaryItem)] \| null` | Нет | Да | `factory: list()` | `—` | — | Значение должно соответствовать одному из типов: list[DictionaryItem], None | Список элементов справочника |
