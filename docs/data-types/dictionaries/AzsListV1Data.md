---
description: "Основные данные списка торговых точек (v1)"
---
# `AzsListV1Data`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `total_count` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Количество найденных торговых точек |
| `result` | `list[AzsItemV1] \| None` | `array[object (AzsItemV1)] \| null` | Нет | Да | `factory: list()` | `—` | — | Значение должно соответствовать одному из типов: list[AzsItemV1], None | Список торговых точек |
