---
description: "Отдельное значение фильтра"
---
# `AzsFilterValue`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `name` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Название значения фильтра |
| `code` | `str \| None` | `string \| null` | Да | Да | `—` | `—` | — | Значение должно соответствовать одному из типов: str, None | Код значения фильтра; реальный API может вернуть null |
