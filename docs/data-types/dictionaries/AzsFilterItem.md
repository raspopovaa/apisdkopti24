---
description: "Описание фильтра торговых точек"
---
# `AzsFilterItem`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `filter` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Ключ фильтра (например: services_with_card, countries и т.д.) |
| `name` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Название фильтра (человекочитаемое) |
| `items` | `list[AzsFilterValue]` | `array[object (AzsFilterValue)]` | Да | Нет | `—` | `—` | — | Проверяется как список; каждый элемент проверяется как AzsFilterValue. | Список значений для данного фильтра |
