---
description: "Группа услуг, доступных на торговой точке"
---
# `ServiceGroup`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `name` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Наименование группы услуг |
| `items` | `list[ServiceItem]` | `array[object (ServiceItem)]` | Да | Нет | `—` | `—` | — | Проверяется как список; каждый элемент проверяется как ServiceItem. | Список услуг, входящих в группу |
