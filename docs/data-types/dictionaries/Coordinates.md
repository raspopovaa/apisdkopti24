...
| `coordinates` | `list[float]` | `array[number]` | Нет | Нет | `фабрика: list()` | `—` | — | Проверяется как список; каждый элемент проверяется как float. | Координаты в формате [долгота, широта] |
