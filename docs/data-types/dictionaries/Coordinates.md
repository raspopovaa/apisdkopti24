---
description: "Географические координаты торговой точки"
---
# `Coordinates`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `type` | `str \| None` | `string \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: str, None | Тип геоданных (обычно 'Point') |
| `coordinates` | `list[float]` | `array[number]` | Нет | Нет | `factory: list()` | `—` | — | Проверяется как список; каждый элемент проверяется как float. | Координаты в формате [долгота, широта] |
