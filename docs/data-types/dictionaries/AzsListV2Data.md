---
description: "Данные списка торговых точек (v2)"
---
# `AzsListV2Data`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `total_count` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Общее количество торговых точек |
| `result` | `list[AzsItemV2]` | `array[object (AzsItemV2)]` | Да | Нет | `—` | `—` | — | Проверяется как список; каждый элемент проверяется как AzsItemV2. | Список торговых точек (АЗС) |
