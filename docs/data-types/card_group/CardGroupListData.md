---
description: "Контейнер данных со списком групп карт."
---
# `CardGroupListData`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `total_count` | `int` | `integer` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как int. | Общее количество групп |
| `result` | `list[CardGroupItem] \| None` | `array[object (CardGroupItem)] \| null` | Нет | Да | `None` | `—` | — | Значение должно соответствовать одному из типов: list[CardGroupItem], None | Список групп карт |
...
- [`CardGroupItem`](CardGroupItem.md)
