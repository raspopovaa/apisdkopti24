---
description: "Информация о группе карт."
---
# `CardGroupItem`
...
|---|---|---|:---:|:---:|---|---|---|---|---|
| `id` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Идентификатор группы карт |
| `name` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Название группы карт |
| `cards_count` | `int \| str` | `integer \| string` | Да | Нет | `—` | `—` | — | Значение должно соответствовать одному из типов: int, str | Количество карт в группе |
| `status` | `str` | `string` | Да | Нет | `—` | `—` | — | Значение преобразуется и проверяется как str. | Статус группы (например, Synchronize) |
