...
def _load_yaml(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ValueError(f"Не удалось загрузить контракт YAML {path}: {exc}") from exc
    if not isinstance(value, dict):
        raise ValueError(f"Контракт YAML должен содержать объект: {path}")
    return value
...
def _field(value: dict[str, Any], *, location: str) -> FieldContract:
    ...
    if not isinstance(path, str) or not path:
        raise ValueError(f"Отсутствует путь поля в {location}")
    if not isinstance(api_type, str) or not api_type:
        raise ValueError(f"Отсутствует api_type для {path} в {location}")
...
def _decision(value: dict[str, Any], *, location: str) -> ContractDecision:
    ...
    if not all(isinstance(item, str) and item for item in (path, status, reason)):
        raise ValueError(f"Недопустимое решение в {location}")
...
def _variant(value: dict[str, Any], *, repository_root: Path, location: str) -> VariantContract:
    ...
    if not isinstance(route_name, str) or not route_name:
        raise ValueError(f"Отсутствует route_name в {location}")
...
def load_catalog(contract_root: Path, *, repository_root: Path | None = None) -> ContractCatalog:
    ...
    if not isinstance(source, dict):
        raise ValueError("Поле source манифеста должно быть объектом")
    source_version = source.get("version")
    if source_version != "1.1.60":
        raise ValueError(f"Неожиданная версия источника контракта: {source_version!r}")
    ...
    if not isinstance(domain_files, list) or not all(
        ...
        raise ValueError("Поле domain_files манифеста должно быть списком строк")
    ...
    for filename in manifest.domain_files:
        ...
        if not isinstance(domain, str) or not domain:
            raise ValueError(f"Отсутствует domain в {domain_path}")
        raw_operations = domain_value.get("operations")
        if not isinstance(raw_operations, dict):
            raise ValueError(f"operations должен быть объектом в {domain_path}")
        for operation_name, raw_operation in raw_operations.items():
            if operation_name in operations:
                raise ValueError(f"Повторяющийся контракт операции: {operation_name}")
            if operation_name in manifest.excluded_operations:
                raise ValueError(
                    f"Исключённая операция не должна быть нормализована: {operation_name}"
                )
            if not isinstance(raw_operation, dict):
                raise ValueError(f"Операция {operation_name} должна быть объектом")
            verification = raw_operation.get("verification")
            if verification not in _ALLOWED_VERIFICATION:
                raise ValueError(
                    f"Неподдерживаемый статус проверки {verification!r} для {operation_name}"
                )
            response_kind = raw_operation.get("response_kind", "pydantic")
            if response_kind not in _ALLOWED_RESPONSE_KINDS:
                raise ValueError(
                    f"Неподдерживаемый вид ответа {response_kind!r} для {operation_name}"
                )
            variants_value = raw_operation.get("variants", [])
            if not isinstance(variants_value, list) or not variants_value:
                raise ValueError(f"Операция {operation_name} должна содержать variants")
            variants = tuple(
                _variant(
                    item,
            ...
            if len(route_names) != len(set(route_names)):
                raise ValueError(f"Повторяющийся вариант маршрута для {operation_name}")
    ...
    if len(operations) != manifest.expected_operation_count:
        raise ValueError(
            "Число нормализованных операций не совпадает с манифестом: "
            f"expected {manifest.expected_operation_count}, получено {len(operations)}"
        )
