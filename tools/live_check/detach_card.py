"""detach_card — Открепление карты от пользователя

Что делает
    Открепляет карту от пользователя. Передаёт user_id и card_id, выводит bool-envelope
    результата.
    Изменяет данные: да. Тарификация: да. Демо-стенд: да.
    Вызов SDK: client.users.detach_card(...)
    Раздел спецификации 1.1.60: Открепление карты от пользователя
...
DESCRIPTION = 'Открепляет карту от пользователя. Передаёт user_id и card_id, выводит bool-envelope результата.'

if __name__ == "__main__":
    run('detach_card', PARAMS, mutating=True, description=DESCRIPTION)
