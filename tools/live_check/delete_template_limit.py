"""delete_template_limit — Удаление лимита шаблона ВК

Что делает
    Удаляет лимит шаблона. Передаёт template_id и limit_id, выводит bool-envelope
    результата.
    Изменяет данные: да. Тарификация: да. Демо-стенд: да.
    Вызов SDK: client.templates.delete_template_limit(...)
    Раздел спецификации 1.1.60: Удаление лимита шаблона ВК

HTTP-запрос
      DELETE /v2/vc/templates/{template_id}/limits/{limit_id}
      POST /v2/vc/templates/{template_id}/limits/{limit_id} (вариант post_override)
...
DESCRIPTION = (
    'Удаляет лимит шаблона. Передаёт template_id и limit_id, выводит bool-envelope результата.'
)

if __name__ == "__main__":
    run('delete_template_limit', PARAMS, mutating=True, description=DESCRIPTION)
