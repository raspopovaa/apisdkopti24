"""resend_invite — Повторная отправка приглашения

Что делает
    Повторно отправляет приглашение. Передаёт invite_id, выводит envelope приглашения.
    Изменяет данные: да. Тарификация: да. Демо-стенд: нет.
    Вызов SDK: client.invites.resend_invite(...)
    Раздел спецификации 1.1.60: Повторная отправка приглашения
...
DESCRIPTION = 'Повторно отправляет приглашение. Передаёт invite_id, выводит envelope приглашения.'

if __name__ == "__main__":
    run('resend_invite', PARAMS, mutating=True, description=DESCRIPTION)
