...
Что делает
    Завершает серверную сессию. Передаёт session_id из клиента, выводит envelope выхода.
    Изменяет данные: нет. Тарификация: нет. Демо-стенд: да.
    Вызов SDK: client.auth.logoff(...)
    Раздел спецификации 1.1.60: Деавторизация пользователя
...
DESCRIPTION = 'Завершает серверную сессию. Передаёт session_id из клиента, выводит envelope выхода.'

if __name__ == "__main__":
    run('logoff', PARAMS, mutating=False, description=DESCRIPTION)
