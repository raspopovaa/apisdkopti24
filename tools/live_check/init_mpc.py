"""init_mpc — Начать выпуск мобильного профиля карты.

Что делает
    Инициализирует MPC/QR для карты. Передаёт карту, пользователя, PIN и данные устройства.
    Изменяет данные: да. Тарификация: нет. Демо-стенд: нет.
    Вызов SDK: client.virtual_cards.init_mpc(...)
...
Параметры метода SDK
      ...
      user_id: str, обязательный — ID пользователя, к которому привязана карта.
      pin: str, обязательный — Новый PIN мобильного профиля из 4–8 цифр.
      device_id: str, обязательный — Идентификатор устройства длиной 1–255 символов.
      device_name: str, обязательный — Название устройства длиной 11–17 символов.
...
Модели проверки входящих данных (запрос)
    MPCInitRequest:
      ...
      user_id: str, обязательное — описание не задано
      device_id: str, обязательное — описание не задано
      device_name: str, обязательное — описание не задано
...
PARAMS = {
    ...
    'pin': method_value('init_mpc', 'pin'),
    'device_id': method_value('init_mpc', 'device_id'),
    'device_name': method_value('init_mpc', 'device_name'),
...
if __name__ == "__main__":
    run('init_mpc', PARAMS, mutating=True, description=DESCRIPTION)
