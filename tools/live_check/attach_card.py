"""attach_card — Прикрепление карты к пользователю

Что делает
    Прикрепляет карту к пользователю. Передаёт user_id и card_id, выводит bool-envelope
    результата.
    Изменяет данные: да. Тарификация: да. Демо-стенд: да.
    Вызов SDK: client.users.attach_card(...)
    Раздел спецификации 1.1.60: Прикрепление карты к пользователю
...
DESCRIPTION = 'Прикрепляет карту к пользователю. Передаёт user_id и card_id, выводит bool-envelope результата.'

if __name__ == "__main__":
    run('attach_card', PARAMS, mutating=True, description=DESCRIPTION)
