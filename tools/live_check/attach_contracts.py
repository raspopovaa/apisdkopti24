"""attach_contracts — Прикрепление договоров к пользователю

Что делает
    Прикрепляет договоры к пользователю. Передаёт user_id и JSON-список contracts, выводит
    bool-envelope результата.
    Изменяет данные: да. Тарификация: да. Демо-стенд: да.
    Вызов SDK: client.users.attach_contracts(...)
    Раздел спецификации 1.1.60: Прикрепление договоров к пользователю
...
Параметры метода SDK
      ...
      contracts: list[UserAttachContractRequest | Mapping[str, object]], обязательный — Список договоров для прикрепления к пользователю. Для каждого договора указываются `sid` — ID договора, `template_id` — ID шаблона ВК, `use_mpc` — разрешение выпуска МПК (`true`/`false`).
...
Модели проверки входящих данных (запрос)
    UserAttachContractRequest:
      ...
      use_mpc: bool | None, необязательное — Разрешён ли выпуск МПК
...
Возможные ошибки
    До отправки запроса (локальные проверки SDK):
      ValueError: contracts должен содержать хотя бы один элемент
...
PARAMS = {
    ...
    'contracts': method_value('attach_contracts', 'contracts'),
}

DESCRIPTION = 'Прикрепляет договоры к пользователю. Передаёт user_id и JSON-список contracts, выводит bool-envelope результата.'

if __name__ == "__main__":
    run('attach_contracts', PARAMS, mutating=True, description=DESCRIPTION)
