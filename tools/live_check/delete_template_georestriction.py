"""delete_template_georestriction — Удаление геоограничителя шаблона ВК

Что делает
    Удаляет геоограничение шаблона. Передаёт template_id и georestriction_id, выводит bool-
    envelope результата.
    Изменяет данные: да. Тарификация: не указано. Демо-стенд: да.
    Вызов SDK: client.templates.delete_template_georestriction(...)
    Раздел спецификации 1.1.60: Удаление геоограничителя шаблона ВК

HTTP-запрос
      DELETE /v2/vc/templates/{template_id}/georestrictions/{georestriction_id}
      DELETE /v2/vc/templates/{template_id}/georestrictions/{georestrictions_id} (вариант plural_id)
      POST /v2/vc/templates/{template_id}/georestrictions/{georestriction_id} (вариант post_override)
...
DESCRIPTION = 'Удаляет геоограничение шаблона. Передаёт template_id и georestriction_id, выводит bool-envelope результата.'

if __name__ == "__main__":
    run('delete_template_georestriction', PARAMS, mutating=True, description=DESCRIPTION)
