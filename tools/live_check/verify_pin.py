"""verify_pin — Запрос одноразового кода для сброса попыток ввода PIN карты

Что делает
    Проверяет PIN карты. Передаёт card_id и contract_id, выводит bool-envelope результата.
    Изменяет данные: да. Тарификация: нет. Демо-стенд: да.
    Вызов SDK: client.cards.verify_pin(...)
    Раздел спецификации 1.1.60: Запрос одноразового кода для сброса попыток ввода PIN карты
...
if __name__ == "__main__":
    run('verify_pin', PARAMS, mutating=True, description=DESCRIPTION)
