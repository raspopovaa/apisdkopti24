"""update_template_restriction — Изменение ограничителя шаблона ВК

Что делает
    Изменяет товарное ограничение шаблона. Передаёт template_id, restriction_id и payload
    JSON, выводит envelope результата.
    Изменяет данные: да. Тарификация: не указано. Демо-стенд: да.
    Вызов SDK: client.templates.update_template_restriction(...)
    Раздел спецификации 1.1.60: Изменение ограничителя шаблона ВК

HTTP-запрос
      POST /v2/vc/templates/{template_id}/restrictions/{restriction_id}
      PUT /v2/vc/templates/{template_id}/restrictions/{restriction_id} (вариант put)
      PUT /v2/vc/templates/{template_id}/restrictions/{restrictions_id} (вариант put_plural_id)
...
Параметры метода SDK
      ...
      payload: TemplateRestrictionCreateRequest | Mapping[str, Any], обязательный — Изменяемые параметры ограничителя: `product_type`, `product_group`, `restriction_type`; `contract_id` изменить нельзя.
...
PARAMS = {
    ...
    'payload': method_value('update_template_restriction', 'payload'),
    'contract_id': common('contract_id'),
    'use_post': True,
}

DESCRIPTION = 'Изменяет товарное ограничение шаблона. Передаёт template_id, restriction_id и payload JSON, выводит envelope результата.'

if __name__ == "__main__":
    run('update_template_restriction', PARAMS, mutating=True, description=DESCRIPTION)
