"""delete_template — Удаление шаблона ВК

Что делает
    Удаляет шаблон виртуальной карты. Передаёт template_id, выводит bool-envelope
    результата.
    Изменяет данные: да. Тарификация: да. Демо-стенд: да.
    Вызов SDK: client.templates.delete_template(...)
    Раздел спецификации 1.1.60: Удаление шаблона ВК

HTTP-запрос
      DELETE /v2/vc/templates/{template_id}
      POST /v2/vc/templates/{template_id} (вариант post_override)
...
DESCRIPTION = (
    'Удаляет шаблон виртуальной карты. Передаёт template_id, выводит bool-envelope результата.'
)

if __name__ == "__main__":
    run('delete_template', PARAMS, mutating=True, description=DESCRIPTION)
