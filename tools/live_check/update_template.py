"""update_template — Изменение шаблона ВК

Что делает
    Изменяет шаблон виртуальной карты. Передаёт template_id, type_, name и contract_id,
    выводит envelope результата.
    Изменяет данные: да. Тарификация: не указано. Демо-стенд: да.
    Вызов SDK: client.templates.update_template(...)
    Раздел спецификации 1.1.60: Изменение шаблона ВК

HTTP-запрос
      POST /v2/vc/templates/{template_id}
      PUT /v2/vc/templates/{template_id} (вариант put)
...
PARAMS = {
    ...
    'type_': method_value('update_template', 'type_'),
    'name': method_value('update_template', 'name'),
    'contract_id': common('contract_id'),
}

DESCRIPTION = 'Изменяет шаблон виртуальной карты. Передаёт template_id, type_, name и contract_id, выводит envelope результата.'

if __name__ == "__main__":
    run('update_template', PARAMS, mutating=True, description=DESCRIPTION)
