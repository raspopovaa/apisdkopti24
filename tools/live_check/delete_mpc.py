"""delete_mpc — Удалить мобильный профиль карты.

Что делает
    Удаляет MPC/QR-привязку карты. Передаёт card_id, выводит envelope результата.
    Изменяет данные: да. Тарификация: нет. Демо-стенд: нет.
    Вызов SDK: client.virtual_cards.delete_mpc(...)
...
Модели проверки исходящих данных (ответ API)
    SimpleActionResponse:
      status: StatusModel, обязательное — Статус выполнения операции
...
DESCRIPTION = 'Удаляет MPC/QR-привязку карты. Передаёт card_id, выводит envelope результата.'

if __name__ == "__main__":
    run('delete_mpc', PARAMS, mutating=True, description=DESCRIPTION)
