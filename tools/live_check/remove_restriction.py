"""remove_restriction — Удаление товарного ограничителя по карте и группе карт

Что делает
    Удаляет товарный ограничитель. Передаёт restriction_id, contract_id и опциональный
    group_id, выводит bool-envelope результата.
    Изменяет данные: да. Тарификация: да. Демо-стенд: да.
    Вызов SDK: client.restrictions.remove_restriction(...)
    Раздел спецификации 1.1.60: Удаление товарного ограничителя по карте и группе карт
...
Параметры метода SDK
      ...
      restriction_id: str, обязательный, тип в спецификации: string,string — ID товарного ограничителя.
      group_id: str | None, необязательный, по умолчанию None, тип в спецификации: string — ID группы карт. Если ID группы карты не передано, будет удален лимит по карте. Если передан ID группы карт, то будет удален лимит по группе карт
...
PARAMS = {
    ...
    'restriction_id': common('restriction_id'),
    'group_id': None,
}

DESCRIPTION = 'Удаляет товарный ограничитель. Передаёт restriction_id, contract_id и опциональный group_id, выводит bool-envelope результата.'

if __name__ == "__main__":
    run('remove_restriction', PARAMS, mutating=True, description=DESCRIPTION)
