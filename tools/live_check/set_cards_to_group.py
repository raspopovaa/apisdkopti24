"""set_cards_to_group — Добавление карт в группу карт

Что делает
    Назначает карты в группу. Передаёт group_id, contract_id и cards_list JSON, выводит
    envelope результата.
    Изменяет данные: да. Тарификация: да. Демо-стенд: да.
    Вызов SDK: client.card_groups.set_cards_to_group(...)
    Раздел спецификации 1.1.60: Добавление карт в группу карт
...
Параметры метода SDK
      ...
      cards_list: list[CardGroupAssignmentRequest | Mapping[str, object]], обязательный, тип в спецификации: json — Список карт договора, добавляемых в группу или удаляемых из неё.
...
Модели проверки входящих данных (запрос)
    CardGroupAssignmentRequest:
      ...
      type: Literal['Attach', 'Detach'], обязательное — Действие с картой
...
Возможные ошибки
    До отправки запроса (локальные проверки SDK):
      ValueError: cards_list должен содержать хотя бы один элемент
...
PARAMS = {
    ...
    'cards_list': method_value('set_cards_to_group', 'cards_list'),
    'contract_id': common('contract_id'),
}

DESCRIPTION = 'Назначает карты в группу. Передаёт group_id, contract_id и cards_list JSON, выводит envelope результата.'

if __name__ == "__main__":
    run('set_cards_to_group', PARAMS, mutating=True, description=DESCRIPTION)
