"""move_to_contract — Перевести деньги с кошелька на договор

Что делает
    Возвращает средства с карты на договор. Передаёт contract_id, card_id и amount, выводит
    envelope результата.
    Изменяет данные: да. Тарификация: да. Демо-стенд: да.
    Вызов SDK: client.ewallet.move_to_contract(...)
    Раздел спецификации 1.1.60: Перевести деньги с кошелька на договор
...
Параметры метода SDK
      ...
      amount: Decimal, обязательный, тип в спецификации: string — Сумма перевода.
...
PARAMS = {
    ...
    'amount': decimal(method_value('move_to_contract', 'amount')),
}

DESCRIPTION = 'Возвращает средства с карты на договор. Передаёт contract_id, card_id и amount, выводит envelope результата.'

if __name__ == "__main__":
    run('move_to_contract', PARAMS, mutating=True, description=DESCRIPTION)
