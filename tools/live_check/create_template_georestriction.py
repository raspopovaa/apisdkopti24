"""create_template_georestriction — Создание геоограничителя шаблона ВК

Что делает
    Создаёт геоограничение в шаблоне. Передаёт template_id и payload JSON, выводит envelope
    с ID геоограничения.
    Изменяет данные: да. Тарификация: да. Демо-стенд: да.
    Вызов SDK: client.templates.create_template_georestriction(...)
    Раздел спецификации 1.1.60: Создание геоограничителя шаблона ВК
...
Параметры метода SDK
      ...
      payload: TemplateGeoRestrictionCreateRequest | Mapping[str, Any], обязательный — Параметры геоограничителя: `contract_id`, `country`, `region`, `partner`, `service_center`, `restriction_type` (`1` — разрешающий, `2` — запрещающий).
...
PARAMS = {
    ...
    'payload': method_value('create_template_georestriction', 'payload'),
    'contract_id': common('contract_id'),
}

DESCRIPTION = 'Создаёт геоограничение в шаблоне. Передаёт template_id и payload JSON, выводит envelope с ID геоограничения.'

if __name__ == "__main__":
    run('create_template_georestriction', PARAMS, mutating=True, description=DESCRIPTION)
