"""download_report_file — Скачать файл сформированного отчета.

Что делает
    Скачивает файл отчёта v2. Передаёт job_id, выводит размер bytes-ответа.
    Изменяет данные: нет. Тарификация: да. Демо-стенд: нет.
    Вызов SDK: client.reports.download_report_file(...)
    Раздел спецификации 1.1.60: Генерация файла отчета (v.2)
...
DESCRIPTION = 'Скачивает файл отчёта v2. Передаёт job_id, выводит размер bytes-ответа.'

if __name__ == "__main__":
    run('download_report_file', PARAMS, mutating=False, description=DESCRIPTION)
