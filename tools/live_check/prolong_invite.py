"""prolong_invite — Продлить приглашение

Что делает
    Продлевает приглашение. Передаёт invite_id и with_send, выводит bool-envelope
    результата.
    Изменяет данные: да. Тарификация: да. Демо-стенд: нет.
    Вызов SDK: client.invites.prolong_invite(...)
    Раздел спецификации 1.1.60: Продлить приглашение

HTTP-запрос
      POST /v2/invites/{invite_id}/prolong
      POST /v2/invites/{invite_id}/prolong_free (вариант without_send)
    Параметры передаются: путь URL.
    Заголовки: api_key, date_time, session_id (после авторизации).

Параметры метода SDK
      invite_id: str, обязательный — Идентификатор приглашения.
...
PARAMS = {
    'invite_id': common('invite_id'),
    'with_send': True,
}

DESCRIPTION = (
    'Продлевает приглашение. Передаёт invite_id и with_send, выводит bool-envelope результата.'
)

if __name__ == "__main__":
    run('prolong_invite', PARAMS, mutating=True, description=DESCRIPTION)
