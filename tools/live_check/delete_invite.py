...
Что делает
    Удаляет приглашение. Передаёт invite_id, выводит bool-envelope результата.
    Изменяет данные: да. Тарификация: нет. Демо-стенд: да.
    Вызов SDK: client.invites.delete_invite(...)
    Раздел спецификации 1.1.60: Удалить приглашение

HTTP-запрос
      DELETE /v2/invites/{invite_id}
      POST /v2/invites/{invite_id} (вариант post_override)
...
DESCRIPTION = 'Удаляет приглашение. Передаёт invite_id, выводит bool-envelope результата.'

if __name__ == "__main__":
    run('delete_invite', PARAMS, mutating=True, description=DESCRIPTION)
