"""set_card_comment — Установить комментарий на топливную карту

Что делает
    Устанавливает комментарий к карте. Передаёт card_id, contract_id и comment, выводит
    bool-envelope результата.
    Изменяет данные: да. Тарификация: да. Демо-стенд: да.
    Вызов SDK: client.cards.set_card_comment(...)
    Раздел спецификации 1.1.60: Установить комментарий на топливную карту
...
Параметры метода SDK
      ...
      comment: str, обязательный, тип в спецификации: string — Комментарий к топливной карте.
...
PARAMS = {
    ...
    'comment': method_value('set_card_comment', 'comment'),
    'contract_id': common('contract_id'),
}

DESCRIPTION = 'Устанавливает комментарий к карте. Передаёт card_id, contract_id и comment, выводит bool-envelope результата.'

if __name__ == "__main__":
    run('set_card_comment', PARAMS, mutating=True, description=DESCRIPTION)
