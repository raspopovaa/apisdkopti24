"""get_card_transactions_v2 — Список транзакций по карте (v.2)

Что делает
    Получает транзакции конкретной карты. Передаёт card_id, contract_id, период и пагинацию,
    выводит страницу транзакций.
    Изменяет данные: нет. Тарификация: да. Демо-стенд: да.
    Вызов SDK: client.transactions.get_card_transactions_v2(...)
    Раздел спецификации 1.1.60: Список транзакций по карте (v.2)
...
Параметры метода SDK
      ...
      page_limit: int, необязательный, по умолчанию 100, тип в спецификации: uint — Количество транзакций на странице; по спецификации — 500, если параметр не указан.
      page_offset: int, необязательный, по умолчанию 0, тип в спецификации: uint — Количество транзакций, которые нужно пропустить.
...
DESCRIPTION = 'Получает транзакции конкретной карты. Передаёт card_id, contract_id, период и пагинацию, выводит страницу транзакций.'

if __name__ == "__main__":
    run('get_card_transactions_v2', PARAMS, mutating=False, description=DESCRIPTION)
