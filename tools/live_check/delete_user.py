...
Что делает
    Удаляет пользователя. Передаёт user_id, выводит bool-envelope результата.
    Изменяет данные: да. Тарификация: да. Демо-стенд: да.
    Вызов SDK: client.users.delete_user(...)
    Раздел спецификации 1.1.60: Удаление пользователя

HTTP-запрос
      DELETE /v2/users/{user_id}
      POST /v2/users/{user_id} (вариант post_override)
...
DESCRIPTION = 'Удаляет пользователя. Передаёт user_id, выводит bool-envelope результата.'

if __name__ == "__main__":
    run('delete_user', PARAMS, mutating=True, description=DESCRIPTION)
