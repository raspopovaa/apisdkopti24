"""detach_contracts — Открепление договоров от пользователя

Что делает
    Открепляет договоры от пользователя. Передаёт user_id и список contracts, выводит bool-
    envelope результата.
    Изменяет данные: да. Тарификация: да. Демо-стенд: да.
    Вызов SDK: client.users.detach_contracts(...)
    Раздел спецификации 1.1.60: Открепление договоров от пользователя
...
Параметры метода SDK
      ...
      contracts: list[str], обязательный — Список ID договоров, которые нужно открепить от пользователя.
...
PARAMS = {
    ...
    'contracts': method_value('detach_contracts', 'contracts'),
}

DESCRIPTION = 'Открепляет договоры от пользователя. Передаёт user_id и список contracts, выводит bool-envelope результата.'

if __name__ == "__main__":
    run('detach_contracts', PARAMS, mutating=True, description=DESCRIPTION)
