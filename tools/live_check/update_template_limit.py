"""update_template_limit — Изменение лимита шаблона ВК

Что делает
    Изменяет лимит шаблона. Передаёт template_id, limit_id и limits JSON, выводит envelope
    результата.
    Изменяет данные: да. Тарификация: не указано. Демо-стенд: да.
    Вызов SDK: client.templates.update_template_limit(...)
    Раздел спецификации 1.1.60: Изменение лимита шаблона ВК

HTTP-запрос
      POST /v2/vc/templates/{template_id}/limits/{limit_id}
      PUT /v2/vc/templates/{template_id}/limits/{limit_id} (вариант put)
...
Параметры метода SDK
      ...
      limit_id: str, обязательный — ID лимита шаблона ВК.
      limits: list[TemplateLimitCreateRequest | Mapping[str, Any]], обязательный — Параметры изменения лимита: ограничение `amount` или `sum`, `time`/`term`, `product_type`, `product_group`; `contract_id` изменить нельзя.
...
Возможные ошибки
    До отправки запроса (локальные проверки SDK):
      ValueError: limits должен содержать хотя бы один элемент
      ValueError: Каждый лимит шаблона должен содержать amount или sum
...
PARAMS = {
    ...
    'limit_id': common('template_limit_id'),
    'limits': method_value('update_template_limit', 'limits'),
    'contract_id': common('contract_id'),
    'use_post': True,
}

DESCRIPTION = 'Изменяет лимит шаблона. Передаёт template_id, limit_id и limits JSON, выводит envelope результата.'

if __name__ == "__main__":
    run('update_template_limit', PARAMS, mutating=True, description=DESCRIPTION)
