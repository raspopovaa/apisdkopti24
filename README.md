...
`error_message` формируется из локального каталога: произвольный текст ответа
сервера и неизвестные типы ошибок не попадают в обычные сообщения и audit.
